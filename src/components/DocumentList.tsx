...
// import { FileText, Trash2, MoreVertical, Edit3 } from 'lucide-react';
...
//             <button
//               className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-full hover:bg-gray-200 focus:outline-none"
//               onClick={e => { e.stopPropagation(); setMenuOpenId(menuOpenId === doc.id ? null : doc.id); }}
//             >
//               <MoreVertical className="w-5 h-5 text-gray-400" />
//             </button>
//             {menuOpenId === doc.id && (
//               <div className="absolute right-0 mt-2 z-20 bg-gray-900 rounded-xl shadow-lg py-2 w-44 border border-gray-700">
//                 <button
//                   className="flex items-center w-full px-4 py-2 text-blue-300 hover:bg-gray-800 hover:text-blue-400 text-sm gap-2"
//                   onClick={() => { deleteDocument(doc.id); setMenuOpenId(null); }}
//                 >
    ...
    <div className="space-y-2">
      {documents.map((doc) => (
        ...
        >
          ...
          {/* Dropdown menu */}
          {menuOpenId === doc.id && (
            <div className="absolute right-3 top-full mt-2 z-30 bg-gray-900 rounded-xl shadow-lg py-2 w-44 border border-gray-700">
              <button
                className="flex items-center w-full px-4 py-2 text-blue-300 hover:bg-gray-800 hover:text-blue-400 text-sm gap-2"
                onClick={e => {
                  e.stopPropagation();
